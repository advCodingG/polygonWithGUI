...
void ofApp::setup(){
    ...
    //the listener listens to the sound button press state;
    soundButton.addListener(this, &ofApp::soundButtonPressed);
    
    //set up the gui settings
    gui.setup();
    gui.setPosition(900, 300);
    gui.setBorderColor(ofColor::yellowGreen);
    gui.setHeaderBackgroundColor(ofColor::blueViolet);
    
    //add a toggle that controls fill and noFill
    gui.add(filled.setup("fill", true));
    filled.setTextColor(ofColor::yellowGreen);
    filled.setBackgroundColor(ofColor::wheat);

    //add a slider that controls the random range of the vertex
    gui.add(randomPoints.setup("Random Points", 100, 0, 200));
    randomPoints.setTextColor(ofColor::yellowGreen);
    randomPoints.setBackgroundColor(ofColor::snow);
    randomPoints.setFillColor(ofColor::turquoise);

    //add a button that plays sound
    gui.add(soundButton.setup("Sound"));
    sound.loadSound("CommedyPunch.wav");
    soundButton.setTextColor(ofColor::yellowGreen);
//    soundButton.setBackgroundColor(ofColor::wheat);
...
void ofApp::draw(){
    ...
    ofBeginShape();
    for (int i = 0; i < 10; i++){
        ofVertex(ofRandom(800 - randomPoints,800 + randomPoints), ofRandom(100 - randomPoints , 100 + randomPoints));
    }
    ofEndShape();
    //-------------------------------------
